...
fn Printer(comptime cfg: Config) type {
    ...
    fn emitJsxElementClassic(self: *Self, e: ast.JSXElement) Error!void {
        const opening = self.tree.data(e.opening_element).jsx_opening_element;
        try self.writeStr("React.createElement(");
        try self.emitJsxTagClassic(opening.name);
        try self.writeStr(", ");
        try self.emitJsxPropsClassic(opening.attributes);
        try self.emitJsxChildrenClassic(e.children);
        try self.writeByte(')');
    }
    ...
    fn emitJsxTagClassic(self: *Self, idx: NodeIndex) Error!void {
        switch (self.tree.data(idx)) {
            .jsx_identifier => |id| {
                const name = self.tree.string(id.name);
                if (name.len > 0 and (std.ascii.isLower(name[0]) or std.mem.indexOfScalar(u8, name, '-') != null)) {
                    try self.emitStringLit(id.name);
                } else {
                    try self.writeString(id.name);
                }
            },
            .jsx_member_expression => |m| {
                try self.emitJsxTagClassic(m.object);
                try self.writeByte('.');
                try self.emit(m.property);
            },
    ...
    fn emitJsxPropsClassic(self: *Self, attrs: IndexRange) Error!void {
        const list = self.tree.extra(attrs);
        if (list.len == 0) {
            try self.writeStr("null");
            return;
        ...
        if (!has_spread) {
            try self.emitJsxPropsObjectClassic(list);
            return;
        }
        ...
        if (object_open) try self.writeByte('}');
        try self.writeByte(')');
    }

    fn emitJsxPropsObjectClassic(self: *Self, list: []const NodeIndex) Error!void {
        try self.writeByte('{');
        var first = true;
        for (list) |idx| {
    ...
    fn emitJsxPropClassic(self: *Self, idx: NodeIndex) Error!void {
        ...
        try self.writeStr(": ");
        if (attr.value == .null) {
            try self.writeStr("true");
            return;
        }

        switch (self.tree.data(attr.value)) {
            .jsx_expression_container => |c| try self.emit(c.expression),
            .jsx_element => |e| try self.emitJsxElementClassic(e),
            .jsx_fragment => |f| try self.emitJsxFragmentClassic(f),
            else => try self.emit(attr.value),
        }
    }

    fn emitJsxPropNameClassic(self: *Self, idx: NodeIndex) Error!void {
        switch (self.tree.data(idx)) {
            .jsx_identifier => |id| try self.emitStringLit(id.name),
            .jsx_namespaced_name => |n| {
                try self.writeByte('"');
                try self.emit(n.namespace);
                try self.writeByte(':');
    ...
    fn emitJsxChildrenClassic(self: *Self, children: IndexRange) Error!void {
        for (self.tree.extra(children)) |idx| {
            switch (self.tree.data(idx)) {
                ...
                .jsx_element => |e| {
                    try self.writeStr(", ");
                    try self.emitJsxElementClassic(e);
                },
                .jsx_fragment => |f| {
                    try self.writeStr(", ");
                    try self.emitJsxFragmentClassic(f);
                },
                .jsx_spread_child => |c| {
                    try self.writeStr(", ...");
                    try self.emit(c.expression);
...
fn isAllAsciiWhitespace(value: []const u8) bool {
    for (value) |char| {
