...
- [x] Expand selector support beyond the current basic selectors and descendant combinators. Start with child combinators, grouped selectors, attribute operators, and the pseudo-classes that Testing Library or WPT pressure exposes first. Add local DOM tests plus a tiny WPT selector slice, then run `bun run verify:dom`, `bun run verify:wpt:tiny`, and `bun run verify:fast`.
- [x] Move the pragmatic `innerHTML` parser/serializer path toward a real HTML fragment parser. Current parser modules under `src/dom/` are placeholders while parsing behavior lives in the JS wrapper path; add support incrementally for entity decoding, void elements, comments, malformed markup recovery, table/template contexts, and WPT fragment cases. Keep React smoke green with `bun run verify:react`.
- [x] Replace compatibility no-ops with observable behavior where needed, starting with `MutationObserver`. The current surface is intentionally no-op for construct/observe/disconnect/takeRecords compatibility; implement queued mutation records for attributes, childList, subtree, and characterData when tests require it.
- [x] Deepen DOM Events/UI Events parity. Capture/target/bubble, property handlers, and common event constructors exist, but edge cases remain around listener options, event subclass fields, composed paths, default actions, disabled controls, form submission, and WPT dispatch semantics. Add WPT event basics before broad changes.
- [x] Expand forms and form-control behavior beyond the current React smoke coverage. Cover checked/value/defaultValue reflection, select/option/textarea, labels, form.elements, submit/reset behavior, disabled controls, and Testing Library role/query expectations.
- [x] Finish Custom Elements and Shadow DOM semantics beyond the current basic compatibility slice. Add constructor timing, upgrade edge cases, observed attributes, connected/disconnected ordering, `ShadowRoot`-specific shape, slot basics if needed, and WPT-backed expected failures for unimplemented lifecycle behavior.
- [x] Add document ownership and cross-document algorithms: `adoptNode`, `importNode`, `ownerDocument` updates during moves, cross-window insertion errors, and clone/import coverage. This should be DOM-core/WPT-driven because regressions here are easy to hide behind passing React smoke tests.
- [x] Broaden the WPT harness from the tiny in-repo subset toward real upstream DOM/Event/HTML fragments. Keep `verify:wpt:tiny` fast, but add explicit manifests for selectors, events, parser fragments, forms, custom elements, and shadow DOM with expected failures that include owner and reason.
- [x] Turn the placeholder Zig DOM modules into real module boundaries or delete them. Most native behavior currently lives in `src/zig_dom.zig`; before the implementation grows much more, split stable pieces into `src/dom/node.zig`, `element.zig`, `document.zig`, `selector.zig`, `parser.zig`, and `serializer.zig` without changing the FFI surface.
- [x] Add leak/debug allocation verification for repeated create/append/remove/drop cycles. The plan calls for proving FinalizationRegistry plus explicit `window.close()` do not leak or double-free; add a narrow debug counter or test-only native probe and keep it out of the daily fast path unless it stays cheap.
- [x] Make error mapping more DOMException-compatible. Native status codes currently become controlled JS errors, but remaining work should map hierarchy, not-found, invalid-state, syntax, and stale-handle cases to DOMException-compatible names/messages and cover them with local tests plus WPT expected behavior.
- [x] Expand compatibility documentation after each new slice. `docs/compatibility.md` is current and verification is green as of 2026-05-03, but the next agent should update implemented APIs, known gaps, WPT counts, benchmark notes, and warm-run timings whenever a todo above changes behavior.
