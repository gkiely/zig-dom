...
function dedent(strings: TemplateStringsArray, ...values: unknown[]): string {
  const markers = values.map((_, index) => `\0${index}\0`);
  const raw = strings.reduce(
    (source, chunk, index) => source + chunk + (markers[index] ?? ""),
    ""
  );
  ...
  const dedented = indentation
    ? lines.map((line) => line.slice(indentation)).join("\n")
    : lines.join("\n");

  return markers.reduce(
    (source, marker, index) => source.replaceAll(marker, String(values[index])),
    dedented
  );
}
...
function parseScriptBlocks(
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    if (src) {
      ...
      if (
        normalized === "/resources/testharness.js" ||
        normalized === "/resources/testharnessreport.js"
      ) {
        continue;
      }
      scripts.push(
        readFileSync(resolveScriptPath(entryFile, src, wptRootPath), "utf8")
      );
    } else if (body.trim()) {
      scripts.push(body);
    }
...
function parseIframeInitScripts(
  entryFile: string,
  html: string,
  wptRootPath: string
  ...
  for (const rawSrc of rawSrcs) {
    ...
    const bodySetup =
      frameBody.length > 0
        ...
        : "";
    scriptsBySrc[srcRef] = [bodySetup, ...frameScripts]
      .filter(Boolean)
      .join("\n;\n");
  }
...
function generateHtmlTest(
  ...
  const scripts = [
    ...parseMetaScripts(html).map((script) =>
      readFileSync(resolveScriptPath(entry.file, script, wptRootPath), "utf8")
    ),
...
const wptRootPath = resolve(
  optionalArg("--wpt-root") ?? ".wpt-cache/web-platform-tests"
);
...
for (const [index, { entry, variant }] of selected.entries()) {
  ...
  const outFile = resolve(
    outDir,
    `wpt-${String(startEntry + index).padStart(5, "0")}.test.js`
  );
  const subtests = Array.from(
    skipExpectedSubtests.get(entry.file.replaceAll("\\", "/")) ?? []
  );
...
for (let offset = 0; offset < generatedFiles.length; offset += batchSize) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    ...
    const flakyGcAssert = combined.includes(
      "Assertion failed: (list_empty(&rt->gc_obj_list))"
    );
    if (flakyGcAssert && attempt < maxRetries) {
      console.warn(
        `NATIVE_WPT retry batch=${batchIndex}/${batchTotal} attempt=${attempt + 1}/${maxRetries}`
      );
      continue;
    }
